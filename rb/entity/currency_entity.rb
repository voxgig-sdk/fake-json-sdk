...
class CurrencyEntity
  ...
  def make
    opts = @_entopts.dup
    CurrencyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FakeJsonHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Currency, Hash] the current Currency data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FakeJsonHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Currency fields)
  def match_get
  ...
  #
  # @param reqmatch [CurrencyListMatch, Hash, nil] match filter (any subset of
  #   Currency fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Currency>, Array] the matching Currency items; raises FakeJsonError on failure
