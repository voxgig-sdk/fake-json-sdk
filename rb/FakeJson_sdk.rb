...
require_relative 'FakeJson_types'
...
class FakeJsonSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FakeJsonHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FakeJsonHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FakeJsonFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FakeJsonHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FakeJsonHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FakeJsonHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FakeJsonError.new(
        "#{op}_allow",
        "FakeJsonSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FakeJsonHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FakeJsonError.new(
        "graphql_error", "FakeJsonSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
