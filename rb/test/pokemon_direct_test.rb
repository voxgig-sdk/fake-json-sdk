...
class PokemonDirectTest < Minitest::Test
  def test_direct_list_pokemon
    setup = pokemon_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-pokemon", setup[:live] ? "live" : "unit")
    if _should_skip
...
def pokemon_direct_setup(mockres)
