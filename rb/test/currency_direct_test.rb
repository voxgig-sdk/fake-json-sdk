...
class CurrencyDirectTest < Minitest::Test
  def test_direct_list_currency
    setup = currency_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-currency", setup[:live] ? "live" : "unit")
    if _should_skip
...
def currency_direct_setup(mockres)
