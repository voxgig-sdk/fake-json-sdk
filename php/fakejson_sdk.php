...
class FakeJsonSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FakeJsonHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FakeJsonHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FakeJsonFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FakeJsonFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FakeJsonUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FakeJsonHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FakeJsonHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FakeJsonHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FakeJsonError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FakeJsonHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FakeJsonError("graphql_error",
                "FakeJsonSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
