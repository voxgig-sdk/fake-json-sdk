...
class PokemonEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Pokemon(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Pokemon(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = pokemon_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "pokemon." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FAKE_JSON_TEST_POKEMON_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $pokemon_ref01_data = null;
        if (count($pokemon_ref01_data_raw) > 0) {
...
function pokemon_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/pokemon/PokemonTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["pokemon01", "pokemon02", "pokemon03"] as $k) {
    ...
    $entid_env_raw = getenv("FAKE_JSON_TEST_POKEMON_ENTID");
