...
class FtCtrl extends FakeJsonControl
{
...
class FtClient
{
    ...
    private FakeJsonUtility $utility;

    public function __construct(FakeJsonUtility $utility, array $options)
    {
    ...
    public function get_utility(): FakeJsonUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FakeJsonContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FakeJsonContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FakeJsonContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FakeJsonConfig::make_feature($name);
    ...
    private static function buildUrl(FakeJsonSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FakeJsonError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FakeJsonResponse($fetched);
        }
    ...
    private function fail(FakeJsonContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FakeJsonContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
