...
{
    private static function utility(): FakeJsonUtility
    {
        return new FakeJsonUtility();
    }
    ...
    private static function ctx(array $over = []): FakeJsonContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FakeJsonContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FakeJsonOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FakeJsonError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FakeJsonOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FakeJsonMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FakeJsonOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FakeJsonMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FakeJsonOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FakeJsonMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FakeJsonSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FakeJsonMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new FakeJsonResult([]),
        ]);
        [, $err] = FakeJsonMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FakeJsonSpec([]),
            'response' => null,
            'result' => new FakeJsonResult([]),
        ]);
        [, $err] = FakeJsonMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FakeJsonSpec(['step' => 's']),
            'response' => new FakeJsonResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FakeJsonResult([]),
        ]);
        [, $err] = FakeJsonMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FakeJsonSpec(['step' => 's']),
            'response' => new FakeJsonResponse(self::resp(200, ['v' => 1])),
            'result' => new FakeJsonResult([]),
        ]);
        [, $err] = FakeJsonMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FakeJsonSpec(['step' => 's']),
            'response' => new FakeJsonResponse(self::resp(200, ['v' => 2])),
            'result' => new FakeJsonResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FakeJsonMakeResponse::call($ctx);
    ...
    {
        $preset = new FakeJsonResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FakeJsonSpec([]),
            'response' => new FakeJsonResponse([]),
            'result' => new FakeJsonResult([]),
        ...
        [$got, $err] = FakeJsonMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FakeJsonResult([])]);
        [, $err] = FakeJsonMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FakeJsonSpec([]), 'result' => null]);
        [, $err] = FakeJsonMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FakeJsonSpec(['step' => 's']),
            'result' => new FakeJsonResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FakeJsonOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FakeJsonMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FakeJsonSpec(['step' => 's']),
            'result' => new FakeJsonResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FakeJsonOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FakeJsonMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FakeJsonResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FakeJsonSpec([]),
            'result' => new FakeJsonResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FakeJsonMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FakeJsonMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FakeJsonError('boom', 'boom');
        $utility->fetcher = function (FakeJsonContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FakeJsonSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FakeJsonMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FakeJsonContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FakeJsonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FakeJsonResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FakeJsonContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FakeJsonSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FakeJsonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FakeJsonResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FakeJsonSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FakeJsonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FakeJsonMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FakeJsonMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FakeJsonResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FakeJsonDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FakeJsonResult(['ok' => false])]);
        $this->expectException(FakeJsonError::class);
        FakeJsonDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FakeJsonResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FakeJsonMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FakeJsonMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FakeJsonBaseFeature();
        $b = new FakeJsonBaseFeature();
        FakeJsonFeatureAdd::call($ctx, $a);
        FakeJsonFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FakeJsonBaseFeature
    {
        $f = new FakeJsonBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FakeJsonFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FakeJsonContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FakeJsonSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FakeJsonPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FakeJsonResponse(['headers' => null]),
            'result' => new FakeJsonResult([]),
        ]);
        FakeJsonResultHeaders::call($ctx);
