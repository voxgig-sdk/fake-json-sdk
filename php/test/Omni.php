...
//    only - a typed FakeJsonContext there would make every ctx
...
final class FakeJsonOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new FakeJsonContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new FakeJsonSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new FakeJsonResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new FakeJsonError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new FakeJsonResponse($clean['response']);
    ...
    public static function err_from_map($m): ?FakeJsonError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FakeJsonError(is_string($code) ? $code : '', $msg);
    }
