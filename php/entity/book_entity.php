...
class BookEntity
{
    ...
    {
        if ($args) {
            $this->_data = FakeJsonHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = FakeJsonHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws FakeJsonError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FakeJsonHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws FakeJsonError on failure (item-5 convention).
     */
     ...
     * Create a new Book.
     *
     * @param BookCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed BookCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Book|array The created Book as an assoc-array at the
     *   SDK boundary; throws FakeJsonError on failure (item-5 convention).
     */
     ...
     *
     * @param BookUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed BookUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Book|array The updated Book as an assoc-array at the
     *   SDK boundary; throws FakeJsonError on failure (item-5 convention).
     */
     ...
     *
     * @param BookRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; BookRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Book|array The removed Book as an assoc-array at the
     *   SDK boundary; throws FakeJsonError on failure (item-5 convention).
     */
