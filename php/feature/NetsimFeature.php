...
class FakeJsonNetsimFeature extends FakeJsonBaseFeature
{
    ...
    private function _simulate(FakeJsonContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FakeJsonContext $ctx, array $applied): void
    {
        $client = $this->client;
