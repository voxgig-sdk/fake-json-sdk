...
func TestPersonEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FAKEJSON_TEST_PERSON_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func personBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FAKEJSON_TEST_PERSON_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FAKEJSON_TEST_PERSON_ENTID"])
