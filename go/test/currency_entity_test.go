...
func TestCurrencyEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Currency(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Currency(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FAKE_JSON_TEST_CURRENCY_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		currencyRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.currency")))
		...
		currencyRef01Match := map[string]any{}

		currencyRef01ListResult, err := currencyRef01Ent.List(currencyRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, currencyRef01ListOk := currencyRef01ListResult.([]any)
		if !currencyRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", currencyRef01ListResult)
		}
...
func currencyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FAKE_JSON_TEST_CURRENCY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FAKE_JSON_TEST_CURRENCY_ENTID"])
