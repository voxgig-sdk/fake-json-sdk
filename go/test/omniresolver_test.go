...
import (
	...
	omni "github.com/voxgig-sdk/fake-json-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FakeJsonError); is {
				out := map[string]any{
					"name":    "FakeJsonError",
...
func omniCtx(arg any, client *sdk.FakeJsonSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FakeJsonSDK); is {
			client = live
