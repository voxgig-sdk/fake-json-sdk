// Command FakeJson-cli is a CLI + REPL for the FakeJson SDK,
...
const prompt = "fake-json"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "book currency person pokemon"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FAKE_JSON_APIKEY (api key), FAKE_JSON_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FAKE_JSON_APIKEY carries the API key and
	// FAKE_JSON_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FAKE_JSON_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FAKE_JSON_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFakeJsonSDK(opts)
