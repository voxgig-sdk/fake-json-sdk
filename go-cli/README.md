...
boru-driven command-line client **and** interactive REPL for the FakeJson
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fake-json-cli)
make build
...
./fake-json-cli load 1 book            # {id:1} shorthand
./fake-json-cli load '{id:1}' book       # explicit match map
./fake-json-cli update '{name:"x"}' book
./fake-json-cli list currency

# 5. Override the API base URL for a single call
FAKE_JSON_BASE=https://api.example.com ./fake-json-cli list book
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fake-json-cli
   ```
...
```sh
./fake-json-cli load 1 book          # scalar shorthand for {id:1}
./fake-json-cli load '{id:1}' book     # explicit match map
```
...
```sh
./fake-json-cli update '{id:1,name:"new"}' book
```
...
Run with no arguments to open a REPL (prompt `fake-json>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `book`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fake-json-cli`. |
...
(`./fake-json-cli <expr>`) and interactively (the REPL), and expressions compose the
