...
class TestPokemonEntity:

    def test_should_create_instance(self):
        testsdk = FakeJsonSDK.test(None, None)
        ent = testsdk.Pokemon(None)
    ...
    def test_should_stream(self):
        ...
        base = FakeJsonSDK.test(seed, None)
        seen = list(base.Pokemon(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = FakeJsonSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Pokemon(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _pokemon_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "pokemon." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FAKE_JSON_TEST_POKEMON_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        pokemon_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.pokemon")))
        pokemon_ref01_data = None
        if len(pokemon_ref01_data_raw) > 0:
...
def _pokemon_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/pokemon/PokemonTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FAKE_JSON_TEST_POKEMON_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FAKE_JSON_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("FAKE_JSON_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FAKE_JSON_TEST_EXPLAIN") == "TRUE",
        "live": _live,
