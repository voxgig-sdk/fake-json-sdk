...
class TestPokemonDirect:

    def test_should_direct_list_pokemon(self):
        setup = _pokemon_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-pokemon", "live" if setup["live"] else "unit")
        if _skip:
...
def _pokemon_direct_setup(mockres):
