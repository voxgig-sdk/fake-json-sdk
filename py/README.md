...
The Python SDK for the FakeJson API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
# Create
created, _ = client.Book(None).create({"name": "Example"}, None)

# Update
client.Book(None).update({"id": created["id"], "name": "Example-Renamed"}, None)

# Remove
client.Book(None).remove({"id": created["id"]}, None)
```
...
result, err = client.FakeJson(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── fakejson_sdk.py         -- Main SDK module
...
The main module (`fakejson_sdk`) exports the SDK class.
