...
from fakejson_sdk.fakejson_types import (
    Pokemon,
    PokemonListMatch,
)
...
class PokemonEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PokemonEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Pokemon]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Pokemon().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
