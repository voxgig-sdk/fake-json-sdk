...
class FakeJsonTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FakeJsonTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FakeJsonTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FakeJsonTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = FakeJsonTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FakeJsonTestRunner.getenv(key)
        ...
        explain = FakeJsonTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FakeJsonTestRunner._test_control is not None:
            return FakeJsonTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FakeJsonTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FakeJsonTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FakeJsonTestRunner.load_test_control()
...
def load_env_local():
    FakeJsonTestRunner.load_env_local()


def env_override(m):
    return FakeJsonTestRunner.env_override(m)


def entity_data(v):
    return FakeJsonTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FakeJsonTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FakeJsonTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FakeJsonTestRunner.load_test_control()


def live_client_options():
    return FakeJsonTestRunner.live_client_options()


def live_delay_ms():
    return FakeJsonTestRunner.live_delay_ms()
