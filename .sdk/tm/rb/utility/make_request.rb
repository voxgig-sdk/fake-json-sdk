...
module FakeJsonUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = FakeJsonResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FakeJsonResponse.new(fetched)
    else
