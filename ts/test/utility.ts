...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FAKEJSON_TEST_LIVE ||
    'TRUE' === process.env.FAKEJSON_TEST_OVERRIDE
  ) {
  ...
  m.FAKEJSON_TEST_EXPLAIN = process.env.FAKEJSON_TEST_EXPLAIN || m.FAKEJSON_TEST_EXPLAIN
