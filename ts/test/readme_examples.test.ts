...
//     (\`new FakeJsonSDK(...)\` / \`FakeJsonSDK.test(...)\` become
//     \`FakeJsonSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"book":{"test01":{"id":"test01"}},"currency":{"test01":{"id":"test01"}},"person":{"test01":{"id":"test01"}},"pokemon":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FakeJsonSDK(...)\` and any
// \`FakeJsonSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FakeJsonSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FakeJsonSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
