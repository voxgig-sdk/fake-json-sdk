...
(0, node_test_1.describe)('BookDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-book', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-book', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-book', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-book', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.FAKE_JSON_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.FakeJsonSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['FAKE_JSON_TEST_BOOK_ENTID'];
    ...
    const client = new __1.FakeJsonSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=BookDirect.test.js.map