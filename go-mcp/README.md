...
[MCP](https://modelcontextprotocol.io) server exposing the FakeJson SDK as
two agent tools — `fake-json_list` and `fake-json_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fake-json-mcp)
make build

# 2. Provide credentials via the environment
export FAKE_JSON_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user fake-json \
  -- /absolute/path/to/fake-json-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fake-json-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fake-json_list: first page of records
{ "entity": "book" }
{ "entity": "book", "query": { } }

// fake-json_load: one record by id
{ "entity": "book", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fake-json-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fake-json \
     -- "$PWD"/dist/*/fake-json-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fake-json_list` and `fake-json_load` tools now appear
   in new sessions. Ask the agent to *"list book using fake-json"*
   and it calls `fake-json_list` with `{"entity":"book"}`.
...
```sh
export FAKE_JSON_APIKEY=sk_live_xxx            # API key
export FAKE_JSON_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `fake-json_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fake-json_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FAKE_JSON_APIKEY` | API key sent with every request. |
| `FAKE_JSON_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

book | currency | person | pokemon

### Smoke test via HTTP (raw JSON-RPC)

```sh
./fake-json-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fake-json_load","arguments":{"entity":"book","query":{"id":1}}}}'
```
