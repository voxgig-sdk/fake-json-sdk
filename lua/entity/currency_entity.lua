...
local CurrencyEntity = {}
CurrencyEntity.__index = CurrencyEntity


function CurrencyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CurrencyEntity)
  self._name = "currency"
  self._client = client
...
function CurrencyEntity:get_name()
  return self._name
end


function CurrencyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CurrencyEntity.new(self._client, opts)
end
...
function CurrencyEntity:mark_deleted()
  self._deleted = true
end


function CurrencyEntity:deleted()
  return true == self._deleted
end


function CurrencyEntity:data_set(args)
  if args ~= nil then
...
function CurrencyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CurrencyEntity:match_set(args)
  if args ~= nil then
...
function CurrencyEntity:match_get()
...
function CurrencyEntity:stream(action, args, callopts)
...
---@param reqmatch CurrencyListMatch
---@param ctrl? table
---@return Currency[]
---@return string? err
function CurrencyEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CurrencyEntity:_run_op(ctx, post_done)
  local utility = self._utility
