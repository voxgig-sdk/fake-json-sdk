...
local PokemonEntity = {}
PokemonEntity.__index = PokemonEntity


function PokemonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PokemonEntity)
  self._name = "pokemon"
  self._client = client
...
function PokemonEntity:get_name()
  return self._name
end


function PokemonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PokemonEntity.new(self._client, opts)
end
...
function PokemonEntity:mark_deleted()
  self._deleted = true
end


function PokemonEntity:deleted()
  return true == self._deleted
end


function PokemonEntity:data_set(args)
  if args ~= nil then
...
function PokemonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PokemonEntity:match_set(args)
  if args ~= nil then
...
function PokemonEntity:match_get()
...
function PokemonEntity:stream(action, args, callopts)
...
function PokemonEntity:_run_op(ctx, post_done)
  local utility = self._utility
