...
local BookEntity = {}
BookEntity.__index = BookEntity


function BookEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BookEntity)
  self._name = "book"
  self._client = client
...
function BookEntity:get_name()
  return self._name
end


function BookEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BookEntity.new(self._client, opts)
end
...
function BookEntity:mark_deleted()
  self._deleted = true
end


function BookEntity:deleted()
  return true == self._deleted
end


function BookEntity:data_set(args)
  if args ~= nil then
...
function BookEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BookEntity:match_set(args)
  if args ~= nil then
...
function BookEntity:stream(action, args, callopts)
...
function BookEntity:_run_op(ctx, post_done)
  local utility = self._utility
