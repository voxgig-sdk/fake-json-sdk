...
local FakeJsonSDK = {}
FakeJsonSDK.__index = FakeJsonSDK
...
FakeJsonSDK._make_feature = _make_feature


function FakeJsonSDK.new(options)
  local self = setmetatable({}, FakeJsonSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FakeJsonSDK:options_map()
...
function FakeJsonSDK:get_utility()
  return Utility.copy(self._utility)
end


function FakeJsonSDK:get_root_ctx()
  return self._rootctx
end


function FakeJsonSDK:prepare(fetchargs)
...
function FakeJsonSDK:direct(fetchargs)
...
-- by passing it.
function FakeJsonSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FakeJsonSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FakeJsonSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FakeJsonSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FakeJsonSDK.new(sdkopts)
  sdk.mode = "test"
